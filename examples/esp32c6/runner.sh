...
# Building ESP32-C6 image
espflash save-image --chip esp32c6 --merge "$outputfilenamecargo" "$outputfilename" 1>&2

# Get stats
esptool.py image_info --version 2 "$outputfilename" 1>&2

# Run in QEMU
$qemuexec -nographic -monitor tcp:127.0.0.1:55555,server,nowait -icount 3 -machine esp32c6 -drive file="$outputfilename",if=mtd,format=raw -serial file:"$logfile" &
